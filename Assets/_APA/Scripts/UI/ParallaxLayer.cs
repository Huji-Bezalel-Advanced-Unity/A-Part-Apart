...
namespace _APA.Scripts
{
    ...
    {
        [Header("Player References")] [Tooltip("Assign the Transform of Player 1.")] [SerializeField]
        private Transform player1Transform;

        [Tooltip("Assign the Transform of Player 2.")] [SerializeField]
        private Transform player2Transform;

        [Header("Parallax Effect Strength")]
        [Tooltip(
            "How much the layer moves relative to the players' midpoint movement. X for horizontal, Y for vertical.")]
        [SerializeField]
        ...
        [SerializeField]
        private bool enableHorizontalOscillation = false;

        [Tooltip(
            "The total horizontal distance the layer will travel due to parallax in one direction before its parallax response inverts.")]
        [SerializeField]
        private float oscillationTravelDistanceX = 5.0f;
        ...
        void Start()
        {
            ...
            {
                Debug.LogError(
                    $"ParallaxLayer '{gameObject.name}': Player transforms are not assigned. Disabling script.", this);
                enabled = false;
                return;
            }
        ...
        void LateUpdate()
        {
            ...
            if (enableHorizontalOscillation)
            {
                ...
                if (distanceFromOscillationOrigin >= oscillationTravelDistanceX)
                {
                    ...
                    {
                        effectiveParallaxDisplacementX = currentOscillationOriginX + oscillationTravelDistanceX -
                                                         layerInitialAnchorPosition.x;
                    }
                    else
                    {
                        effectiveParallaxDisplacementX = currentOscillationOriginX - oscillationTravelDistanceX -
                    ...
                    Debug.Log(
                        $"ParallaxLayer '{gameObject.name}': Oscillation limit reached. New DirectionX: {currentParallaxDirectionX}, New OscillationOriginX: {currentOscillationOriginX}");
                }
                else
                {
